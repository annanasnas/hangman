...
public:
  ...
  string &returnWord() { return wordToGuess; }
  string &returnHint() { return hint; }
};
 ...
 *
 */
string PrintMessage(string content, bool printTop = true,
 ...
 *
 */
void PrintWordAndCheckWin(string wordToGuess, string wordFromUser, string hint,
                          bool &won, string &UsersWordAndHint) {
  bool realWon = true;
...
int TriesLeft(string wordToGuess, string wordFromUser) {
  ...
  for (int i = 0; i < wordFromUser.length(); i++) {
    if (wordToGuess.find(wordFromUser[i]) == string::npos)
      error++;
  }
  return error;
}
 ...
 *
 */
void StartNewGame(int &tries, bool &won, string &UsersWordAndHint,
                  string &wordFromUser, Database &wordAndHint) {
      ...
      if (event.type == sf::Event::TextEntered) {
        ...
        if (event.text.unicode < 128) {
          ...
          if (!won) {
            ...
            if (wordFromUser.find(letterFromUser) == string::npos)
        ...
        // если пользователь выиграл, он должен нажать "enter" и начать новую
        // игру
        if (event.text.unicode == 10) {
          if (won)
            StartNewGame(tries, won, UsersWordAndHint, wordFromUser,
                         wordAndHint);
        }
      }
    }
